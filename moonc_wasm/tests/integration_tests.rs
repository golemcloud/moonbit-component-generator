...
use moonc_wasm::cross_platform::{
    CrossPlatformMetadataExt, PermissionsBuilder, RawFdExt, host_isatty,
};
...
mod file_operations_integration {
    ...
    fn test_standard_io_descriptors(#[case] fd: i32, #[case] name: &str) {
        ...
        assert!(
            tty_result == 0 || tty_result == 1,
            "{} host_isatty should return 0 or 1",
            name
        );
...
mod metadata_integration_tests {
    ...
    fn test_complete_metadata_workflow(#[case] filename: &str, #[case] content: &str) {
        let temp_dir = TempDir::new().expect("Failed to create temp dir");
        ...
        println!(
            "✅ Complete metadata workflow for {} - {}",
            filename, stat_object
        );
    }
    ...
    fn test_directory_metadata() {
        ...
        println!(
            "✅ Directory metadata - Kind: {}, Mode: 0o{:o}, Links: {}",
            kind, mode, nlink
        );
    }
}
...
mod performance_integration {
    ...
    fn test_metadata_performance_integration() {
        ...
        assert!(
            avg_time < 1000,
            "Average metadata extraction should be under 1ms per file"
        );

        println!(
            "✅ Performance integration - {} files in {:?} (avg: {}μs per file)",
